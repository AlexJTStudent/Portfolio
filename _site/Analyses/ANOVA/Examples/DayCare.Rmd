...
```
```{r}
late_clean <- Late %>% mutate(Period = case_when(Week <= 4 ~ "Before the fine",
                                               Week > 4 & Week < 17 ~  "Fine Applied", TRUE ~ "Fine Removed"))
```

```{r}
table(late_clean$Period)
```

```{r}
warp.aov <- aov(NumberofLateChildren ~ late_clean,data = late_clean)
summary(warp.aov)
```
