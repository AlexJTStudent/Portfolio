...
ggplot(Rent, mapping = aes(x = Longitude, y = Latitude, color = Gender, size = ParkingSpaces)) + 
 geom_point()
...
pander(favstats(Rent$ParkingSpaces ~ Rent$Gender))
```
...
After visualizing the data and making a summary table on parking spaces based on gender, We have can see that females lower(Q1) quarterly is lower than males by 11 spaces. Thus implying that Females tend to have less parking spots than male complexes, opening opportunities to make a case study to find what classifies as a parking spot and what doesn't.
...
```{r}
boxplot(KidsFeet$length ~ KidsFeet$sex, data=KidsFeet)
```

```{r}
KidsFeet %>% group_by(sex) %>%
  summarise(min  = min(sex),
            Q1 = ,
            Q2 = ,
            Q3 = ,
            max = max(sex))
...
```{r}
plot(KidsFeet$length ~ KidsFeet$width,data = KidsFeet)
```
