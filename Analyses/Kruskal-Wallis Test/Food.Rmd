...
library(readr)
library(ggthemes)
food <- read_csv("C://Users/Alex/Documents/GitHub/IntStats/Statistics-Notebook-master/Data/food.csv") #food.csv is in the Data folder...
```
...
```{r}
pander(favstats(weight ~ eating_out, data=food))
```

```{r, fig.align='center'}
pander(kruskal.test(weight ~ eating_out, data=food))
```
...
This analysis fully abstained from using AI, the only references used were
the "Pig Birth Weights – Kruskal-Wallis" from the "Statistics Notebook" guide given by the course.
